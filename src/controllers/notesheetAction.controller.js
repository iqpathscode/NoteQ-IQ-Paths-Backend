...
export const getReceivedNotesheets = async (req, res) => {
  try {
    ...
    const notesheets = await Notesheet.find(finalQuery).sort({
      createdAt: -1,
    });
...
export const getQueriesByNoteId = async (req, res) => {
  try {
    ...
    const normalizedNoteId = isNaN(Number(noteId)) ? noteId : Number(noteId);
...
export const sendQuery = async (req, res) => {
  try {
    ...
    if (!lastActionStep) {
      return res
        .status(400)
        .json({ success: false, message: "No previous handler found" });
    }
...
export const replyQuery = async (req, res) => {
  try {
    ...
    if (!currentQueryStep) {
      return res
        .status(403)
        .json({ success: false, message: "No query assigned to you" });
    }

    // Ensure remark is an array for multi-message
    if (!Array.isArray(currentQueryStep.remark)) {
      currentQueryStep.remark = currentQueryStep.remark
        ? [currentQueryStep.remark]
        : [];
    }
    ...
    return res
      .status(500)
      .json({ success: false, message: "Error replying query" });
  }
};
